...
<head>
    ...
    <link rel="stylesheet" href="/mz_reader/resources/layui/css/layui.css">

    <script src="/mz_reader/resources/wangEditor.min.js"></script>
...
<script src="/mz_reader/resources/layui/layui.all.js"></script>
<script>
    ...
    table.render({
        ...
        , url: "/mz_reader/management/book/list" //数据接口
        , page: true //开启分页
        , cols: [[ //表头
    ...
    function showUpdate(obj){
        ...
        editor.customConfig.uploadImgServer = '/mz_reader/management/book/upload' //设置图片上传路径
		...
		//发送ajax请求,获取对应图书信息
        $.get("/mz_reader/management/book/id/" + bookId , {} , function(json){
			//文本框回填已有数据
    ...
    function showCreate(){
        ...
        editor.customConfig.uploadImgServer = '../book/upload';//设置图片上传地址
    ...
    layui.form.on('submit(btnSubmit)', function(data){
		...
		//向服务器发送请求
        $.post("/mz_reader/management/book/" + optype , formData , function(json){
            if(json.code=="0"){
    ...
    function showDelete(obj){
        ...
        layui.layer.confirm('确定要执行删除操作吗?', {icon: 3, title:'提示'}, function(index){
				...
				$.get("/mz_reader/management/book/delete/" + bookId, {}, function (json) {
					if(json.code=="0"){
						//删除成功刷新表格
						table.reload('bookList');
						//提示操作成功
